...
private_.loaded = !1,
private_.delegates = [],
private_.cacheDelegates = {
    height: 0,
    delegates: []
},
private_.keypairs = {},
private_.outTransferManager = null,
private_.signTransaction = function(e, t) {
    return modules.logic.transaction.create(t, e.keypair)
},
private_.loop = function(e, t) {
    if (!private_.loaded) return app.logger.debug("Loop exit: syncing"),
    (0, _setImmediate3.
default)(t);
    var r = slots.getSlotNumber(),
    a = modules.blockchain.blocks.getLastBlock();
    if (r == slots.getSlotNumber(a.timestamp)) return (0, _setImmediate3.
default)(t);
    var n = private_.getState(a.height + 1);
    if (null === n) return app.logger.info("Loop exit: skipping slot"),
    (0, _setImmediate3.
default)(t);
    library.sequence.add(function(t) { (0, _asyncToGenerator3.
    ...
    default.mark(function r() {
        ...
        default.wrap(function(r) {
                ...
                case 0:
                    ...
                    return r.next = 6,
                    private_.balanceSync(n.keypair);
                case 6:
                    return r.next = 8,
                    private_.withdrawalSync(n.secret);
                case 8:
                ...
                case 11:
                    return r.next = 13,
                    modules.blockchain.blocks.createBlock(n.keypair, n.slotTime, e);
                case 13:
                    i = modules.blockchain.blocks.getLastBlock(),
                    app.logger.info("New dapp block id: " + i.id + " height: " + i.height + " via point: " + i.pointHeight);
                case 15:
                    r.next = 20;
                    break;
                case 17:
                    ...
                    app.logger.error("Failed to create new block: ", r.t0);
                case 20:
...
default.mark(function e(t) {
    ...
    default.wrap(function(e) {
            ...
            case 0:
                return app.logger.trace("enter balanceSync ------------------------"),
                e.next = 3,
            ...
            case 3:
                return r = e.sent,
                app.logger.debug("balanceSync found deposit transactions:", r),
                a = null,
                r && r.length && (a = JSON.parse(r[0].args)[2]),
                e.next = 9,
                PIFY(modules.api.dapps.getBalanceTransactions)(a);
            case 9:
            ...
            case 12:
                app.logger.debug("balanceSync mainTransactions:", n),
                s = [],
                i = !0,
                o = !1,
                l = void 0,
            ...
            case 42:
                ...
                e.next = 45,
                modules.blockchain.transactions.receiveTransactionsAsync(d);
            case 45:
            case "end":
                return e.stop()
            }
        },
        e, this, [[17, 30, 34, 42], [35, , 37, 41]])
    }));
    return function(t) {
...
default.mark(function e(t) {
    ...
    default.wrap(function(e) {
            for (;;) switch (e.prev = e.next) {
            case 0:
                for (r = private_.outTransferManager.getPending(), a = !0, n = !1, s = void 0, e.prev = 4, i = (0, _getIterator3.
            default)(r); ! (a = (o = i.next()).done); a = !0)(l = o.value).signatures.length >= app.meta.unlockDelegates && (modules.api.dapps.submitOutTransfer(l), private_.outTransferManager.setReady(l.id));
                e.next = 12;
                break;
            case 8:
                e.prev = 8,
            ...
            case 20:
                return e.next = 22,
                PIFY(modules.api.dapps.getWithdrawalLastTransaction)();
            case 22:
            ...
            case 28:
                p = e.sent,
                app.logger.debug("found last inner withdrawal", p),
                p ? c = p.height: app.logger.warn("WARNING last inner withdrawal not found", u.id);
            case 31:
                return e.next = 33,
                app.model.Transaction.findAll({
                    condition: {
            ...
            case 33:
                ...
                }), f = !0, h = !1, v = void 0, e.prev = 39, m = (0, _getIterator3.
            default)(g); ! (f = (y = m.next()).done); f = !0) _ = y.value,
                b = _.ot,
                k = _.innerId,
                b.signatures.length >= app.meta.unlockDelegates ? (modules.api.dapps.submitOutTransfer(b), private_.outTransferManager.addReady(b, k)) : (modules.api.transport.message("pendingOutTransfer", b), private_.outTransferManager.addPending(b, k));
                e.next = 47;
                break;
            case 43:
                e.prev = 43,
        ...
        e, this, [[4, 8, 12, 20], [13, , 15, 19], [39, 43, 47, 55], [48, , 50, 54]])
    }));
    return function(t) {
        return e.apply(this, arguments)
    }
...
Round.prototype.onBind = function(e) {
    ...
    app.config.secrets) {
        ...
        app.logger.info("Forging enable on account: " + a),
        private_.keypairs[a] = {
            keypair: r,
...
Round.prototype.onBlockchainLoaded = function() {
    private_.loaded = !0,
    private_.delegates = [];
    for (var e = 0; e < app.meta.delegates.length; e++) private_.delegates.push(modules.blockchain.accounts.generateAddressByPublicKey(app.meta.delegates[e])),
    private_.delegates.sort();
    slots.setDelegatesNumber(app.meta.delegates.length),
    private_.outTransferManager = new OutTransferManager(app.meta.unlockDelegates)
},
Round.prototype.onMessage = function(e) {
    if (private_.loaded) if ("point" == e.topic) {
        var t = e.message;
        private_.loop(t,
        function(e) {

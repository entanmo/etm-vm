...
function Blocks(e, t) {
    ...
    try {
        ...
        return app.logger.error("Failed to load genesis.json"),
        void process.exit(3)
    }
    private_.lastBlock = private_.genesisBlock,
    e(null, self)
}
private_.lastBlock = null,
private_.genesisBlock = null,
private_.loaded = !1,
private_.blockCache = {},
private_.proposeCache = {},
private_.lastPropose = null,
private_.lastVoteTime = null,
private_.deleteBlock = function(e, t) {
...
private_.popLastBlock = function(e, t) {
    ...
    self.getBlock(function(r, o) {
        ...
        o = self.readDbRows(o);
        var a = 0;
        async.eachSeries(e.transactions.reverse(),
        function(e, t) {
            async.series([function(t) {
                a += e.fee,
                modules.blockchain.transactions.undo(e, t)
            },
            function(t) {
        ...
        function(r) {
            r && (app.logger.error(r), process.exit(0)),
...
default.mark(function e(t) {
    ...
    default.wrap(function(e) {
            ...
            case 0:
                ...
                return app.logger.error("verify block undefined"),
                e.abrupt("return");
            case 4:
            ...
            case 9:
                ...
                new Error("Failed to verify block id: " + e.t0);
            case 12:
            ...
            case 17:
                ...
                new Error("Failed to verify signature: " + e.t1);
            case 20:
                if (!t.delegates) {
                    e.next = 22;
                    break
            ...
            case 35:
                ...
                new Error("Invalid payload hash");
            case 38:
                app.logger.trace("before verify transaction signature"),
                r = crypto.createHash("sha256"),
                o = 0,
                e.prev = 41,
                e.t3 = _regenerator2.
            default.keys(t.transactions);
            case 43:
                if ((e.t4 = e.t3()).done) {
                    e.next = 54;
                    break
            ...
            case 56:
                ...
                new Error("Failed to verify transaction: " + e.t5);
            case 59:
...
private_.getIdSequence = function(e, t) {
    var r = this; (0, _asyncToGenerator3.
default)(_regenerator2.
...
private_.rollbackUntilBlock = function(e, t) {
    ...
    function(e, r) { ! e && r.length ? (app.logger.info("Blocks#rollbackUntilBlock", r), self.deleteBlocksBefore(r[0], t)) : t()
    })
},
private_.verifyVotes = function(e) {
    ...
    for (var o = 0; o < e.signatures.length; ++o) {
        ...
        if (!r[a.key]) return app.logger.warn("Votes key is not in the top list: " + a.key),
        !1;
        if (!modules.logic.consensus.verifyVote(e.height, e.id, a)) return app.logger.warn("Failed to verify vote"),
        !1
    }
    return ! 0
},
Blocks.prototype.processFee = function(e) {
    if (e && e.transactions) {
        ...
        o = void 0;
        try {
            for (var a, n = (0, _getIterator3.
        default)(e.transactions); ! (t = (a = n.next()).done); t = !0) {
                var s = a.value,
...
default.mark(function e(t, r) {
    ...
    default.wrap(function(e) {
            ...
            case 0:
                ...
                return e.prev = 2,
                modules.logic.block.normalize(t),
                e.next = 6,
                private_.verify(t);
            case 6:
                if (!r.votes) {
            ...
            case 19:
                ...
                app.logger.error("Failed to verify block: " + e.t0),
                e.t0;
            case 23:
                return app.logger.trace("before applyBlock"),
                e.prev = 24,
                e.next = 27,
                self.applyBlock(t, r);
            case 27:
            ...
            case 29:
                ...
                app.logger.error("Failed to apply block: " + e.t1),
                e.t1;
            case 33:
                return e.prev = 33,
                self.processFee(t),
            ...
            case 42:
                ...
                app.logger.error("save block error: ", e.t2),
                app.sdb.rollbackBlock(),
                new Error("Failed to save block: " + e.t2);
            case 47:
                app.logger.info("Block applied correctly with " + t.count + " transactions"),
                self.setLastBlock(t),
                private_.blockCache = {},
                private_.proposeCache = {},
                private_.lastVoteTime = null,
                modules.logic.consensus.clearState(),
                r.broadcast && modules.api.transport.message("block", {
                    block: t,
                    votes: r.votes
                });
            case 54:
...
default.mark(function e(t) {
    ...
    default.wrap(function(e) {
            ...
            case 3:
                for (app.logger.debug("----------------------on round end-----------------------"), app.logger.debug("app.delegate.length", r.length), o = new _map2.
            default, a = new _map2.
            default, n = app.feePool.getFees(), app.logger.debug("round fees", n), n.forEach(function(e, t) {
                    ...
                    o.forEach(function(e, r) {
                        app.logger.debug("apply round distributing fee", t, r, e),
                        app.balances.increase(t, r, e)
                    }),
                    e === r.length - 1 && a.forEach(function(e, r) {
                        app.logger.debug("apply round distributing fee remain", t, r, e),
                        app.balances.increase(t, r, e)
                    })
                },
...
Blocks.prototype.setLastBlock = function(e) {
    app.logger.trace("Blocks#setLastBlock", e),
    private_.lastBlock = e;
...
Blocks.prototype.saveBlock = function(e) {
    for (var t in app.logger.trace("Blocks#save height", e.height), e.transactions) {
        ...
        r.height = e.height,
        app.sdb.create("Transaction", r)
    }
    var o = {};
    for (var a in e)"transactions" !== a && (o[a] = e[a]);
    app.sdb.create("Block", o),
...
Blocks.prototype.readDbRows = function(e) {
    ...
    r = [], o = 0, a = e.length; o < a; o++) {
        ...
        if (n) {
            ...
            t[n.id].transactions = t[n.id].transactions || {},
            s && (t[n.id].transactions[s.id] || (t[n.id].transactions[s.id] = s))
        }
    }
    return t = r.map(function(e) {
        return t[e].transactions = (0, _keys2.
    default)(t[e].transactions).map(function(r) {
...
Blocks.prototype.deleteBlocksBefore = function(e, t) {
    ...
    function(e) {
        app.logger.trace("Blocks#popLastBlock", private_.lastBlock.height),
        private_.popLastBlock(private_.lastBlock,
        function(t, r) {
            t || (private_.lastBlock = r),
            e(t)
        })
    },
    function(e) { (0, _setImmediate3.
    default)(t, e)
    })
},
...
Blocks.prototype.createBlock = function() {
    var e = (0, _asyncToGenerator3.
default)(_regenerator2.
default.mark(function e(t, r, o, a) {
        var n, s, i, c, l, p, u, d, g, f, h;
        return _regenerator2.
    default.wrap(function(e) {
            for (;;) switch (e.prev = e.next) {
            case 0:
                n = modules.blockchain.transactions.getUnconfirmedTransactionList(),
            ...
            case 25:
                app.logger.info("Forged new block id: " + u.id + " height: " + u.height),
                e.next = 42;
                break;
            case 28:
                "0.0.0.0:0",
                e.prev = 29,
                h = modules.logic.consensus.createPropose(t, u, "0.0.0.0:0"),
                e.next = 37;
                break;
            case 33:
                return e.prev = 33,
                ...
                app.logger.error("Failed to create propose: " + e.t2.toString()),
                e.abrupt("return");
            case 37:
                modules.logic.consensus.setPendingBlock(u),
                modules.logic.consensus.addPendingVotes(f),
                private_.proposeCache[h.hash] = !0,
                app.logger.info("Initiate propose on block id: " + u.id + ", height: " + u.height),
                modules.api.transport.message("propose", h);
            case 42:
            case "end":
                return e.stop()
            }
...
default.mark(function e(t, r) {
    ...
    default.wrap(function(e) {
            ...
            case 11:
                return e.next = 13,
                modules.logic.transaction.apply(n, t);
            case 13:
                o[n.id] = n,
                e.next = 5;
                break;
            ...
            case 18:
                ...
                app.logger.error("apply block error: " + e.t2),
                app.sdb.rollbackBlock(),
                new Error("Failed to apply block: " + e.t2);
            case 23:
            case "end":
                return e.stop()
            }
...
Blocks.prototype.loadBlocksPeer = function(e, t, r) {
    app.logger.info("Load blocks after:", e),
    modules.api.transport.getPeer(t, "get", "/blocks/after", {
        lastBlockHeight: e
    },
    function(e, t) {
...
default.mark(function o() {
    ...
    default.wrap(function(r) {
            ...
            case 11:
                ...
                r.abrupt("return", t("Failed to find common block: " + r.t0));
            case 14:
            case "end":
                return r.stop()
            }
...
default.mark(function e(t, r, o) {
    ...
    default.wrap(function(e) {
            ...
            case 0:
                return a = t,
                e.next = 3,
                PIFY(private_.getIdSequence)(a);
            case 3:
                return n = e.sent,
                app.logger.debug("Blocks#getIdSequence", n),
                s = a,
                a = n.firstHeight,
                i = {
                    ids: n.ids,
                ...
                e.next = 10,
                PIFY(modules.api.transport.getPeer)(r, "get", "/blocks/common", i);
            case 10:
                if ((c = e.sent).body) {
                    e.next = 13;
                    break
            ...
            case 15:
                ...
                c.body.prevBlockId && (l.prevBlockId = c.body.prevBlockId),
                e.next = 19,
                app.model.Block.findOne({
                    condition: l
                });
            case 19:
                if (p = e.sent) {
...
Blocks.prototype.votes = function(e, t) {
    ...
    library.bus.message("votes", e.query.votes),
    t()
},
...
Blocks.prototype.getBlock = function(e, t) {
    var r = e.query;
    modules.api.sql.select(extend({},
    library.scheme.selector.blocks, {
        condition: {
            "b.id": r.id
        },
...
default.mark(function o() {
    ...
    default.wrap(function(r) {
            ...
            case 10:
                ...
                r.abrupt("return", t("System error: " + r.t0));
            case 13:
            case "end":
                return r.stop()
            }
...
default.mark(function o() {
    ...
    default.wrap(function(r) {
            ...
            case 7:
                return app.logger.debug("Blocks#getBlocksAfter get blocks", e, s),
                i = s[s.length - 1].height,
                r.next = 11,
            ...
            case 11:
                for (p in c = r.sent, app.logger.debug("Blocks#getBlocksAfter get transactions", c), l = s[0].height, c) u = c[p],
                d = u.height,
                (g = s[d - l]) && (g.transactions || (g.transactions = []), g.transactions.push(u));
                t(null, {
                    blocks: s
                });
            case 16:
...
Blocks.prototype.onMessage = function(e) {
    if (app.logger.debug("Blocks#onMessage last block = %j, event = %j", private_.lastBlock, e), "block" == e.topic) library.sequence.add(function(t) {
        ...
        o = e.message.block,
        a = e.message.votes;
        a ? o.prevBlockId == private_.lastBlock.id && o.id != private_.lastBlock.id && o.id != private_.genesisBlock.id && o.height == private_.lastBlock.height + 1 ? (0, _asyncToGenerator3.
    default)(_regenerator2.
    default.mark(function e() {
        ...
        default.wrap(function(e) {
                ...
                case 7:
                    ...
                    n = !1,
                    app.logger.error("Blocks#processBlock error", e.t0);
                case 11:
                    if (n) for (s in o.transactions) modules.blockchain.transactions.removeUnconfirmedTransaction(o.transactions[s].id);
                    return e.prev = 12,
                    i = modules.blockchain.transactions.getUnconfirmedTransactionList(),
                    modules.blockchain.transactions.clearUnconfirmed(),
                    e.next = 17,
                    modules.blockchain.transactions.receiveTransactionsAsync(i);
                case 17:
                    e.next = 22;
                    break;
                case 19:
                    ...
                    app.logger.error("Failed to redo unconfirmed transactions", e.t1);
                case 22:
                    t();
                case 23:
                case "end":
    ...
    default)(t):
        app.logger.warn("realtime block consensus must have votes")
    });
        ...
        if (t.height != private_.lastBlock.height + 1) return app.logger.warn("invalid propose height", t),
        void(t.height > private_.lastBlock.height + 1 && app.logger.warn("receive discontinuous propose height " + t.height));
        ...
        if (app.logger.debug("get active keypairs length", r.length), r && r.length > 0) {
            ...
            private_.lastVoteTime = Date.now(),
            private_.lastPropose = t,
            app.logger.info("send votes height " + o.height + " id " + o.id + " sigatures " + o.signatures.length),
            modules.api.transport.message("votes", o)
        }
        ...
        library.sequence.add(function(e) {
            ...
            if (t && t.signatures && app.logger.info("receive new votes, total votes number " + t.signatures.length), modules.logic.consensus.hasEnoughVotes(t)) {
                var r = a.height,
                n = a.id;
                t.signatures = t.signatures.slice(0, Math.min(6, Math.ceil(2 * slots.delegates / 3))),
                (0, _asyncToGenerator3.
            default)(_regenerator2.
            default.mark(function o() {
                    return _regenerator2.
                default.wrap(function(o) {
                        ...
                        case 3:
                            app.logger.info("Forged new block id: " + n + " height: " + r),
                            o.next = 9;
                            break;
                        case 6:
                            o.prev = 6,
                            ...
                            app.logger.error("Failed to process block", {
                                block: a,
                                error: o.t0
                            });
                        case 9:
...
default.mark(function e() {
    ...
    default.wrap(function(e) {
            ...
            case 0:
                return e.prev = 0,
                e.next = 3,
                PIFY(modules.api.dapps.getDApp)();
            case 3:
                return app.meta = e.sent,
                app.logger.info("app.meta", app.meta),
                e.next = 7,
                app.model.Block.count();
            case 7:
                ...
                return e.next = 12,
                self.processBlock(private_.genesisBlock, {});
            case 12:
                e.next = 18;
                break;
            case 14:
            ...
            case 21:
                ...
                app.logger.error("Failed to prepare local blockchain", e.t0),
                process.exit(0);
            case 25:
            case "end":
                return e.stop()
